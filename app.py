        ...
        if isin and (trigger_search or isin):
            if not valida_isin(isin): st.error("❌ ISIN non valido")
                ...
                if d:
                    ...
                    with col_set2:
                        Importo User = st.number_input(f"Capitale da Investire ({valuta_user})", value=10000.0, step=1000.0)
                    with col_set3:
                    ...
                    # Calcoli Flussi
                    nominale_bond = Importo User * tasso_spot 
                    commissioni = 5.0 
...
# --- GRAFICO BREAKEVEN (LINEA BICOLORE + PUNTO PAREGGIO) ---
                    ...
                    # 1. Calcolo Cumulato
                    df_flussi['Cumulativo'] = df_flussi['Importo User'].cumsum()
                    ...
                    st.dataframe(
                        df_flussi[['Data', 'Tipo', 'Importo', 'Importo User', 'Dettagli']]
                        .style
                        .format({
                            'Importo': f'{{:+,.2f}} {valuta_bond}', 
                            'Importo User': f'{{:+,.2f}} {valuta_user}'
                        })
                        .map(style_cedola, subset=['Importo', 'Importo User']),
                        use_container_width=True,
                        height=400
                    )
